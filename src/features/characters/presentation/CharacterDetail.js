...
export default function CharacterDetail({ ...props }) {
  ...
  function getAllAssignedLocation() {
    if (selectedLocation !== "-") {
      onValue(
        ref(firebaseDB, `assign/location-${selectedLocation}`),
        (snapshot) => {
  ...
  async function handleOnClickAssign(e) {
    if (selectedLocation !== "-") {
      ...
      let characterRef = ref(
        firebaseDB,
        `assign/character-${param.characterID}`
      );
      onValue(
        characterRef,
        async (snapshot) => {
          ...
          if (!!data) {
            ...
            await set(
              ref(firebaseDB, `assign/character-${param.characterID}`),
              param
            ).catch((err) => console.log(err));
            await set(
              ref(
                firebaseDB,
                `assign/location-${param.locationID}/${param.characterID}`
              ),
              true
            ).catch((err) => console.log(err));
  ...
  return (
    <div className="col-12 d-flex vh-100 justify-content-center" style={{ backgroundColor: "black" }}>
